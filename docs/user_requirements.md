...
当前活动版本：`v14pro`。本文的规则适用于当前版本及后续版本；底部带日期的版本段落是
...
- 当前配置：`configs/v13pro.yaml` 是套件模板；逐实验配置由 `run_v13pro_suite.py` 生成在输出目录，旧配置留在旧分支。
- 当前代码结构：根目录下的 `data/`、`models/`、`scripts/`、`configs/`、`outputs/`。
- 历史输出显示项目已经迭代到 v9/v9c；创建本文档时尚未检测到 `outputs/outputs_v10a/`。

## 需求变更历史

当前新增要求（2026-09-15）：论文补充实验代码命名为 `v14pro` 并从已推送的 `v13pro`
继承；保留类别绑定、batch 128、Value+cue 和恢复梯度隔离。v14pro 必须提供 CCF-B
双轨实验入口：v13pro 主协议、speaker/OOD 泛化和严格真实 paired manifest；未经实际
训练或缺少第二数据集/外部基线的补充实验标记待运行，不能把 CPU 测试或工程实现宣称为
已满足 CCF-B 录用标准。
