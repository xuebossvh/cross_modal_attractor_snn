...
当前活动版本：`v13pro`。本文的规则适用于当前版本及后续版本；底部带日期的版本段落是
...
当前新增要求（2026-09-15）：论文补充实验代码命名为 `v13pro` 并推送同名分支；保留
类别绑定、batch 128、Value+cue 和恢复梯度隔离。未经实际训练的补充实验标记待运行，
不能把 CPU 测试或工程实现宣称为已满足 CCF C 录用标准。
