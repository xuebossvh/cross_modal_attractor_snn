...
│   └── outputs_v11b/  # 当前主实验产物（Recovery/消融各有独立目录）
│       ├── logs/
│       ├── figures/
│       └── tables/
...
当前官方配置族为 **v11b**。先运行 `v11b_recovery` 恢复 coarse audio；
`v11b_weighted` 从同一第100轮里程碑验证归一化 energy weighting；再从选定的
第120轮父 checkpoint 分叉 `v11b_control`、`v11b_cross_no_causal` 和主实验
`v11b`。后三者只训练 Decoder/Cross-Key adapter，使用相同30轮预算。

```bash
pip install -r requirements.txt
python scripts/mkdir_outputs.py --config configs/v11b_recovery.yaml
nohup env OMP_NUM_THREADS=1 PYTHONUNBUFFERED=1 python -u scripts/train.py --config configs/v11b_recovery.yaml > outputs/outputs_v11b_recovery/logs/train_v11b_recovery_120ep.log 2>&1 < /dev/null &
tail -f outputs/outputs_v11b_recovery/logs/train_v11b_recovery_120ep.log

# recovery 生成 _ep100.pt 后可运行归一化 weighted 分支
python scripts/mkdir_outputs.py --config configs/v11b_weighted.yaml
python -u scripts/train.py --config configs/v11b_weighted.yaml

# 选定第120轮父 checkpoint 后，统一检查三份配置的 init_ckpt_path 再分叉
python -u scripts/train.py --config configs/v11b_control.yaml
python -u scripts/train.py --config configs/v11b_cross_no_causal.yaml
python -u scripts/train.py --config configs/v11b.yaml
```
...
- **readout 阶段**：关闭 target。v11b 先把对侧 cue 的 Key rate 投影为 Value 空间
  residual，与 `v_*_from_A` 相加；再与对应 cue 的同模态 detail state 拼接后送入
  Decoder。缺少对侧 cue 时 residual 严格为 0。随后计算分类 / 图像恢复 / 音频恢复 /
  脉冲正则损失。

每个配置使用独立 checkpoint/output_version。Recovery 还会保存
`cross_modal_snn_v11b_recovery_ep100.pt`，其中包含 model、optimizer、scheduler 与 epoch。

> 注意：分支初始化默认 strict。evaluate 遇到缺失或结构不匹配的
> checkpoint 会直接报错，不会用随机权重生成伪评估结果。

快速冒烟（小子集、1 epoch）：编辑 `configs/v11b_recovery.yaml` 设 `data.train_subset: 512`、
`train.epochs: 1`，再运行 `python -u scripts/train.py`。

## 4. 评估与 Demo

```bash
python -u scripts/evaluate.py --config configs/v11b.yaml --protocol fixed_mask --severity 0.4 --family_breakdown | tee outputs/outputs_v11b/logs/eval_v11b_fixed_mask_sev04.log
python -u scripts/evaluate.py --config configs/v11b.yaml --protocol fixed_mask --severity 0.4 --family_breakdown --cross_key sweep 2>&1 | tee outputs/outputs_v11b/logs/eval_v11b_cross_key_sweep_sev04.log
python -u scripts/evaluate.py --config configs/v11b.yaml --protocol legacy_random | tee outputs/outputs_v11b/logs/eval_v11b_random.log
python -u scripts/demo_inference.py --config configs/v11b.yaml --num 10 --severity 0.4
...
- `evaluate.py`：8 种 cue 模式下的 acc / 图像 MSE·PSNR·SSIM / **log-mel MSE** 等；Cross-Key sweep 同时比较 correct/zero/wrong-class/same-class different-sample Key。
  ...
  `smp`=样本级 / `cat`=类别代表原型）。快速试跑：`python -u scripts/evaluate.py --config configs/v11b.yaml --max_batches 1`。
- `demo_inference.py` 输出三张图，标题明确区分恢复粒度，每格标注
  cue type / target type / true label / pred label / confidence：
  - `outputs/outputs_v11b/figures/demo_aud_only.png`：audio-only cue → **category** image + **sample** audio
  - `outputs/outputs_v11b/figures/demo_img_only.png`：image-only cue → **sample** image + **category** audio
  - `outputs/outputs_v11b/figures/demo_both.png`：双模态 cue → **sample** image + **sample** audio
  - random 可视化会输出 `demo_aud_only_random.png` / `demo_img_only_random.png` / `demo_both_random.png`
  - 评估表：`outputs/outputs_v11b/tables/demo_eval_table.txt`
  - 全量 eval 表格图（按 family 子目录）：`tables/family01_occlusion_time_mask/full_eval.png` 等；
    生成：`python scripts/plot_eval_summary.py outputs/outputs_v11b/logs/eval_v11b_fixed_mask_sev04.log`
...
`configs/v11b_recovery.yaml :: cue_modes`。

损坏函数（`data/corruption.py`，`severity∈[0,1]`）：
- v11b 图像训练/主评估 family：`occlusion` / `pixel_delete` /
  `mask_vertical` / `mask_horizontal` / `salt_mask`。
- v11b 音频训练/主评估 family：`time_mask` / `freq_mask` /
  `feature_dropout` / `partial_temporal` / `time_freq_block`。
- `gaussian` 与方向化 `mask_left|right|top|bottom` 仍可由损坏函数单独调用，
  但不进入 v11b 的五 family 主结论。

## 8. 消融开关（`configs/v11b_recovery.yaml :: ablation`）
