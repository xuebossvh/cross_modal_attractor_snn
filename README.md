...
```bash
pip install -r requirements.txt
python scripts/mkdir_outputs.py --config configs/v6a.yaml
python -u scripts/train.py --config configs/v6a.yaml 2>&1 | tee outputs/outputs_v6a/logs/train_v6a_50ep.log
...
每个 epoch 保存 checkpoint 至 `outputs/checkpoints/cross_modal_snn_v6a.pt`（由 yaml 指定）。
日志 / 图表 / 表格写入 `outputs/outputs_v6a/{logs,figures,tables}/`。

> 注意：若你曾用旧架构训练过，旧 checkpoint 结构不兼容，evaluate/demo 会自动
> 检测并回退到随机权重并给出警告——重新训练即可。

快速冒烟（小子集、1 epoch）：编辑 `configs/v6a.yaml` 设 `data.train_subset: 512`、
`train.epochs: 1`，再运行 `python -u scripts/train.py`。

## 4. 评估与 Demo

```bash
python -u scripts/evaluate.py --config configs/v6a.yaml | tee outputs/outputs_v6a/tables/full_eval_v6a.txt
...
- `demo_inference.py` 输出三张图，标题明确区分恢复粒度，每格标注
  ...
  - `outputs/outputs_v6a/figures/demo_aud_only.png`：audio-only cue → **category** image + **sample** audio
  - `outputs/outputs_v6a/figures/demo_img_only.png`：image-only cue → **sample** image + **category** audio
  - `outputs/outputs_v6a/figures/demo_both.png`：双模态 cue → **sample** image + **sample** audio
  - 评估表：`outputs/outputs_v6a/tables/demo_eval_table.txt`
...
### 关键设计：杜绝答案泄漏
- decoder **永远只读 `v_*_from_A`**（Index 驱动的 Value），从不读
  「A + Encoder 混合」Value。
...
## 8. 消融开关（`configs/v6a.yaml :: ablation`）
