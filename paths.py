...
目录约定：
  ...
  outputs/outputs_v6b/figures/   版本专属图表
  outputs/outputs_v6b/logs/
  outputs/outputs_v6b/tables/
"""
...
DEFAULT_CONFIG = CONFIGS_DIR / "v6b.yaml"


def _normalize_version_folder(tag):
    """'v5' / '5' / 'outputs_v6b' -> 'outputs_v6b'。"""
    tag = str(tag).strip()
...
def infer_output_version(cfg):
    """从 train.output_version 或 configs/v6b.yaml 推断版本目录名。"""
    train = cfg.get("train", {})
